...
use NexaCore\Paymentgateway\Base\RecurringSupport;
use NexaCore\Paymentgateway\Facades\NcPaymentGateway;
use NexaCore\Paymentgateway\Traits\ConvertUsdSupport;
use NexaCore\Paymentgateway\Traits\CurrencySupport;
use NexaCore\Paymentgateway\Traits\IndianCurrencySupport;
use NexaCore\Paymentgateway\Traits\MyanmarCurrencySupport;
...
class Senangpay extends PaymentGatewayBase implements RecurringSupport
{
    ...
    //    dd($response,request()->all());
        if ($response->ok()){
            ...
            if ($data->payment_info?->status !== "failed" && !empty($transaction_reference)){
                ...
                return [
                    'status' => 'complete',
                    "order_id" => NcPaymentGateway::unwrapped_id(request()->get('order_id')),
       ...
       return ['status' => 'failed',"order_id" => NcPaymentGateway::unwrapped_id(request()->get('order_id'))];
    }
    ...
    public function charge_customer(array $args)
    {
        ...
        $order_id =  NcPaymentGateway::wrapped_id($args['order_id']);
    ...
    public function charge_customer_recurring(array $args)
    {
        ...
        $order_param = 'payment_type_###'.$args['payment_type'].'##'.NcPaymentGateway::wrapped_id($args['order_id']);
        $amount = $this->charge_amount($args['amount']);
    ...
    public function ipn_response_recurring(array $args = [])
    {
        ...
        if ($response->ok()){
            ...
            if ($data->payment_info?->status !== "failed" && !empty($transaction_reference)){
                ...
                return [
                    'status' => 'complete',
                    "order_id" => NcPaymentGateway::unwrapped_id($order_id),
                    "payment_type" => $payment_type,
                    'transaction_id' => $transaction_reference
                ];
            }
